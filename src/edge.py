...
class Edge(Optimizer):
    ...
    def _init_group(self, group, params, grads, prevs, step_sizes):
        for p in group["params"]:
            ...
            grad = p.grad
            if grad.is_sparse:
                raise RuntimeError("Edge does not support sparse gradients")
...
Edge.__doc__ = r"""Implements the resilient backpropagation algorithm.
...
def edge(
    ...
    *,
    ...
    r"""Functional API that performs edge algorithm computation.

    See :class:`~torch.optim.Edge` for details.
    """
    ...
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_edge
    else:
        func = _single_tensor_edge
